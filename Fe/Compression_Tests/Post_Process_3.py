...
# Loading all results
FilePathArr = ['SS_curve_Fe_100.txt', 'SS_curve_Fe_110.txt', 'SS_curve_Fe_111.txt']
...
# literature values
lit_val_Fe_E = np.array([127.8, 214.3, 276.6])

# for stress x only
for i in range(0, numfile):
    axs[i].plot(Res_Read[i, :, 0], Res_Read[i, :, 1])
    # plot literature values
    axs[i].plot(Res_Read[i, :, 0], Res_Read[i, :, 0]*lit_val_Fe_E[i],'r')
    # figure titles and axes labels
    axs[i].set_title(Figure_Titles[i])
    axs[i].set_xlabel('Strain')
    axs[i].set_ylabel('Stress')
...
plt.savefig(DirPath + 'results_fig_Fe.png', format = 'png')
